...
DROP MATERIALIZED VIEW api.mystreet2_search CASCADE;
...
SELECT all_projects.project_number as id,
       ...
       setweight(
         ...
         'A') ||
       setweight(
         to_tsvector(coalesce(all_projects.new_project_tags,'')),
         'B')  ||
       setweight(
...
GROUP BY id, name, all_projects.description, all_projects.new_project_tags;
